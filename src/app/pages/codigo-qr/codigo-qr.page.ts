...
export class CodigoQrPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.getQR().subscribe(qrs => {
      ...
      if(qrs['data'].length){
        let aux = qrs['data'][0];
        let duration:any = moment.duration((moment(aux['fecha']).valueOf()-moment().valueOf()),'milliseconds');
        ...
        if((duration-1000) > 1){
          ...
          this.qrData = JSON.stringify({id_QR:this.infoQR['id'],fecha:this.infoQR['fecha']});
  ...
  registrarCodigoQR(){
    ...
    var data= {};
    data['fecha'] = moment().add(2,'hours').format();
  ...
  iniciarContador(){
    if(this.infoQR['creado']){
      ...
      var duration:any = moment.duration((moment(this.authService.infoQR['fecha']).valueOf()-moment().valueOf()),'milliseconds');
